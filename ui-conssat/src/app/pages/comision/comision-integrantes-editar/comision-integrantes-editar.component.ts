...
@Component({
  selector: 'app-comision-integrantes-editar',
  templateUrl: './comision-integrantes-editar.component.html',
  styleUrls: ['./comision-integrantes-editar.component.scss']
})
export class ComisionIntegrantesEditarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.consejeroForm = this.fb.group({
      ...
      vtipoDocumento: [{value: '', disabled: true}, Validators.required],
    ...
    this.comisionesService.buscarIntegrantePorId(this.idIntegrante).subscribe(
      (data: ComisionConsejoResponse) => {
        ...
        this.consejeroForm.setValue({
          ...
          tipo: data.consejero.tipoconsejero.tPconsejeroidpk + '',
  ...
  guardar() {
    ...
    formData.append('codcomiconsejero', this.idIntegrante + '');
    formData.append('vtipoconsejero', this.consejeroForm.get('tipo').value);
    formData.append('vfechainicio', this.datePipe.transform(this.consejeroForm.get('fechaInicio').value, 'dd-MM-yyyy'));
    formData.append('vfechafin', this.datePipe.transform(this.consejeroForm.get('fechaFin').value, 'dd-MM-yyyy'));
    formData.append('vnumerodocumento', this.consejeroForm.get('documentoDesginado').value);
    formData.append('vdocumento', this.archivoAdjunto);

    this.spinnerService.show();
    this.comisionesService.actualizarIntegrantesComision(formData).subscribe(
      (data: ComisionConsejoResponse) => {
        console.log(data);
        setTimeout(() => {
          this.spinnerService.hide();
